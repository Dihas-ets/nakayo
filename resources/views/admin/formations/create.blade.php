...
<div class="max-w-7xl mx-auto">
    <form action="{{ isset($offre) ? route('admin.formations.update', $offre->id_formation) : route('admin.formations.store') }}" method="POST">
        @csrf
        @if(isset($offre)) @method('PUT') @endif

        <div class="flex items-center justify-between mb-10">
            <div class="flex items-center gap-4">
                <a href="{{ route('admin.formations.index') }}" class="w-10 h-10 rounded-full bg-white border border-gray-100 flex items-center justify-center text-gray-400 hover:text-[#1B2E58] transition-all shadow-sm">
                    <i class="fa-solid fa-arrow-left"></i>
                </a>
                <h2 class="text-3xl font-black text-[#1B2E58]">{{ isset($offre) ? 'Modifier' : 'Nouveau' }} Programme</h2>
            </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-2 space-y-6">
                <div class="bg-white rounded-[3rem] p-10 shadow-sm border border-gray-100">
                    ...
                    <div>
                        <label class="block text-xs font-black text-gray-400 uppercase mb-4 tracking-widest">Description du programme</label>
            ...
            <div class="space-y-6">
                <div class="bg-white rounded-[2.5rem] p-8 shadow-sm border border-gray-100 space-y-6">
                    ...
                    <x-input label="Date / Période" name="date_formation" type="date" value="{{ isset($offre->date_formation) ? \Carbon\Carbon::parse($offre->date_formation)->format('Y-m-d') : '' }}" />
        ...
        <option value="non disponible" {{ (isset($offre) && $offre->status == 'non disponible') ? 'selected' : '' }}>
            ❌ Non disponible
                ...
                <div class="bg-[#1B2E58] rounded-[2.5rem] p-10 text-white shadow-xl relative overflow-hidden group">
                    ...
                    <p class="text-xs opacity-60 leading-relaxed italic relative z-10">Assurez-vous de lier la formation au bon pôle d'expertise pour que les clients la retrouvent facilement dans les services correspondants.</p>
